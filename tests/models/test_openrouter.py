...
with try_import() as imports_successful:
    ...
    from pydantic_ai.models.openrouter import OpenRouterModel, OpenRouterResponsesModel
    ...
    def create_openrouter_responses_model(model_name: str, mock_client: Any) -> OpenRouterResponsesModel:
        """Helper to create OpenRouterResponsesModel with mock client using provider pattern."""
        provider = OpenRouterProvider(openai_client=mock_client)
        return OpenRouterResponsesModel(model_name, provider=provider)
...
async def test_openrouter_responses_model_init(allow_model_requests: None):
    """Test basic initialization of OpenRouterResponsesModel."""
    from pydantic_ai.providers.openrouter import OpenRouterProvider

    provider = OpenRouterProvider(api_key='test-key')
    model = OpenRouterResponsesModel('openai/gpt-4o', provider=provider)
    assert model.model_name == 'openai/gpt-4o'
    assert model.system == 'openrouter'
    assert model.client.base_url == 'https://openrouter.ai/api/v1/'


def test_openrouter_responses_model_init_with_string_provider():
    """Test OpenRouterResponsesModel initialization with string provider."""
    with patch.dict(os.environ, {'OPENROUTER_API_KEY': 'test-api-key'}, clear=False):
        model = OpenRouterResponsesModel('openai/gpt-4o', provider='openrouter')
        assert model.model_name == 'openai/gpt-4o'
        assert model.system == 'openrouter'
        assert model.client is not None


async def test_openrouter_responses_basic_request(allow_model_requests: None):
    """Test basic non-streaming request with OpenRouterResponsesModel."""
    mock_response = response_message(
        [
            ResponseOutputMessage(
                ...
                status='completed',
                content=[
                    ResponseOutputText(type='output_text', text='Hello from OpenRouter Responses API!', annotations=[])
                ],
            )
        ]
    )
    ...
    model = create_openrouter_responses_model('openai/gpt-4o', mock_client)
    ...
    assert isinstance(response.parts[0], TextPart)
    assert response.parts[0].content == 'Hello from OpenRouter Responses API!'

    assert response.usage is not None


async def test_openrouter_responses_model_properties(allow_model_requests: None):
    """Test OpenRouterResponsesModel properties and initialization."""
    from pydantic_ai.providers.openrouter import OpenRouterProvider
    ...
    model2 = create_openrouter_responses_model('openai/o1', mock_client)
    assert model2.model_name == 'openai/o1'
    assert model2.client is mock_client


async def test_openrouter_responses_stream(allow_model_requests: None):
    """Test streaming with OpenRouterResponsesModel."""
    ...
    stream_events = [
        ResponseCreatedEvent.model_construct(
            ...
            response=responses.Response(
                ...
                tool_choice='auto',
                tools=[],
                usage=ResponseUsage.model_construct(input_tokens=0, output_tokens=0, total_tokens=0),
            ),
        ),
        ...
        ResponseCompletedEvent.model_construct(
            ...
            response=responses.Response(
                ...
                tool_choice='auto',
                tools=[],
                usage=ResponseUsage.model_construct(input_tokens=10, output_tokens=8, total_tokens=18),
            ),
        ),
    ]
    ...
    assert output_text == ['The capital of France is Paris.']
