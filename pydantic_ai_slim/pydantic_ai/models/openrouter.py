...
__all__ = ['OpenRouterModel', 'OpenRouterResponsesModel']
...
_OPENROUTER_RESPONSES_FINISH_REASON_MAP: dict[str, FinishReason] = {
...
@dataclass(init=False)
class OpenRouterResponsesModel(Model):
    """A model that uses the OpenAI Responses API via OpenRouter.

    This allows using OpenAI models (like o1, o3, gpt-4o) through OpenRouter
    with the Responses API, which provides better reasoning support and tool handling.

    Only works with OpenAI models routed through OpenRouter (e.g., 'openai/gpt-4o', 'openai/o1-preview').
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize an OpenRouter Responses model.

        Args:
            model_name: The name of the OpenRouter model to use (e.g., 'openai/gpt-4o', 'openai/o1-preview').
            provider: The provider to use for authentication and API access. Can be either the string
                'openrouter' or an instance of `Provider[AsyncOpenAI]`. If not provided, a new provider will be
                created using environment variables.
            profile: The model profile to use. Defaults to a profile picked by the provider based on the model name.
    ...
    def _process_response(self, response: responses.Response) -> ModelResponse:
        return process_responses_response(
            ...
            response,
            map_usage_fn=map_usage,
            finish_reason_map=_OPENROUTER_RESPONSES_FINISH_REASON_MAP,
        )
    ...
    def _get_reasoning(self, model_settings: ModelSettings) -> Reasoning | NotGiven:  # pragma: no cover
    ...
    def _map_tool_definition(self, f: ToolDefinition) -> responses.FunctionToolParam:  # pragma: no cover
        return map_responses_tool_definition(self.profile, f)

    def _get_previous_response_id_and_new_messages(  # pragma: no cover
        self, messages: list[ModelMessage]
    ) -> tuple[str | None, list[ModelMessage]]:
        return get_responses_previous_response_id_and_new_messages(messages, self.system)

    async def _map_messages(  # pragma: no cover
